...
function is_slow_instruction(i) {
  return /push/.test(i[0]) || /mov/.test(i[0]) || i[1].some((a) => a.type === "address" || a.type === "memory address");
}
...
function print(instrs, max = 20) {
  ...
  for (const instr of instrs) {
    ...
    const args = instr[1];
    const execute = is_slow_instruction(instr)
      ? // (args.some(a => a.type === "memory address") || /mov/.test(command))
        " X "
      : " x ";
    const wait = args.flatMap(arg_values).some((a) => lastArgs.flatMap(arg_values).some((b) => value_equals(a, b))) ? " ".repeat(width) : "";
  ...
  for (const instr of instrs) {
    ...
    tr.appendChild(
      (() => {
        ...
        const argsStr = args
          .map((a) => {
            const hasDependency = arg_values(a).some((a) => lastArgs.flatMap(arg_values).some((b) => depends_on(a, b)));
            if (hasDependency) {
